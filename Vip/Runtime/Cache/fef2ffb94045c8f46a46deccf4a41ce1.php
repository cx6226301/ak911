...
<div class="accounttitle"><h1>账户转账 </h1></div>
    <form name="form1" method="post" action="__URL__/transferMoneyAC" onSubmit="{return CheckForm();}">
      <table width="100%" border="0" cellpadding="3" cellspacing="0">
          ...
          <tr>
            <td align="right">佣金账户： </td>
            <td colspan="2" width="80%"><span class="hong"><?php echo ($rs["agent_use"]); ?></span></td>
          </tr>
		  <tr>
            <td align="right">金币账户： </td>
            <td colspan="2" width="80%"><span class="hong"><?php echo ($rs["agent_zz"]); ?></span></td>
          </tr>
          <tr>
            <td align="right">类型：</td>
            <td colspan="2">
            <select name="select" id="select" onchange="Selev(this.value)" onpropertychange="Selev(this.value)">
              <option value="1"> 佣金 转给 其它会员 </option>
              <option value="2"> 佣金 转 金币 </option>
              <option value="3"> 佣金 转 报单币 </option>
              <option value="5"> 报单币 转给 其它会员 </option>
			  <option value="4"> 报单币 转 金币 </option>
...
<table width="100%" class="tab3" border="0" cellpadding="3" cellspacing="1" id="tb1" bgcolor="#b9c8d0">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center">
        ...
        <td><?php if(($vo['type']) == "1"): ?>佣金转给其它会员<?php else: endif; ?>
        	<?php if(($vo['type']) == "2"): ?>佣金转金币账户<?php else: endif; ?>
        	<?php if(($vo['type']) == "3"): ?>佣金转报单币<?php else: endif; ?>
        	<?php if(($vo['type']) == "4"): ?>报单币转金币<?php else: endif; ?>
                <?php if(($vo['type']) == "5"): ?>报单币转给其它会员<?php else: endif; ?>
        </td>
