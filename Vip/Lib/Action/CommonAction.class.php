...
//		$fck->ap_wlf();
		...
		$fck->execute("UPDATE __TABLE__ SET `b0`=b1+b2+b3+b4+b5+b6+b7+b8+b9+b10");
...
//		$fwhere['b0'] = array('neq',0);       //这一期总奖金大于0
		...
		$fwhere['_string'] = "b0<>0 or b1<>0 or b2<>0 or b3<>0 or b4<>0 or b5<>0 or b6<>0 or b7<>0 or b8<>0 or b9<>0 or b10<>0";   //已开通的会员
		$rs = $fck ->where($fwhere)->field('*')->order('id asc')->select();
		foreach ($rs as $rss){
			$my_b = $rss['b0'];
			$my_id = $rss['id'];
			$myww = "id=".$my_id." and b0=".$my_b." and is_tj=0";
			$result = $fck -> execute("update __TABLE__ set agent_use=agent_use+".$my_b.",zjj=zjj+b11+b12+b13+b14+b15+b16+b17+b18+b19,is_tj=1 where ".$myww);
			if($result){
				...
				$fck -> query("update __TABLE__ set b0=0,b1=0,b2=0,b3=0,b4=0,b5=0,b6=0,b7=0,b8=0,b9=0,b10=0,b11=0,b12=0,b13=0,b14=0,b15=0,b16=0,b17=0,b18=0,b19=0,b20=0,b25=0,is_tj=0 where id=".$my_id);
