...
$display=$_SESSION[C('_l')]=='english'?'auditMenber_en':'';
            $this->display($display);
        } else {
            $this->error('数据错误!');
            exit;
...
//1111111111
    public function adminMenber($GPid = 0) {
        ...
        if ($_SESSION['UrlPTPass'] == 'MyssGuanShuiPuTao') {
            ...
            $display=$_SESSION[C('_l')]=='english'?'adminMenber_en':'';
            $this->display($display);
            return;
        } else {
            $this->error('数据错误!');
    ...
    public function adminuserData() {
        if ($_SESSION['UrlPTPass'] == 'MyssGuanShuiPuTao' || $_SESSION['UrlPTPass'] == 'MyssGuanXiGua' || $_SESSION['UrlPTPass'] == 'MyssGuansingle' || $_SESSION['UrlPTPass'] == 'MyssShenShuiPuTao') {
            ...
            if ($vo) {
                ...
                $fee = M('fee');
                $fee_s = $fee->field('str29,s10')->find();
                $bank = explode('|', $fee_s['str29']);
                $u_level = explode('|', $fee_s['s10']);
                $this->assign('bank', $bank);
                $this->assign('u_level', $u_level);
                $this->assign('b_bank', $vo);
$display=$_SESSION[C('_l')]=='english'?'adminuserData_en':'';
                $this->display($display);
            } else {
                $this->error('数据错误!');
                exit;
...
//            $data['is_lock']          = $_POST['isLock'];
            ...
            $data['cpzj'] = $s9[$data['u_level'] - 1];
            $data['f4'] = $s2[$data['u_level'] - 1];
            $data['agent_kt'] = $_POST['AgentKt'];
            $data['agent_xf'] = $_POST['AgentXf'];
            $data['agent_gp'] = $_POST['AgentGp'];
            $data['agent_cf'] = $_POST['agent_cf'];
            $data['agent_zz'] = $_POST['agent_zz'];
            $data['live_gupiao'] = $_POST['live_gupiao'];
            $data['wt1'] = $_POST['wt1'];
            $data['da1'] = $_POST['da1'];
...
//========================================出纳管理
    ...
    public function adminFinance() {
        if ($_SESSION['UrlPTPass'] == 'MyssMiHouTao') {
            ...
            $display=$_SESSION[C('_l')]=='english'?'adminFinance_en':'';
            $this->display($display);
        } else {
            $this->error('错误!');
            exit;
...
//            }
            ...
            $display=$_SESSION[C('_l')]=='english'?'adminFinanceList_en':'';
            $this->display($display);
        ...
        if ($_SESSION['UrlPTPass'] == 'MyssPiPa') {
            ...
            if (isset($_REQUEST['FanNowDate'])) {  //日期查询
                if (!empty($_REQUEST['FanNowDate'])) {
                    ...
                    $sql = "where and e_date >= $time1 and e_date <= $time2";
                }
            }
...
$display=$_SESSION[C('_l')]=='english'?'adminFinanceTable_en':'';
            $this->display($display);
        } else {
            $this->error('错误');
            exit;
...
$display=$_SESSION[C('_l')]=='english'?'adminFinanceTableShow_en':'';
            $this->display($display);
        } else {
            $this->error('错误');
            exit;
...
//            dump($history);exit;
            ...
            $fee_s7 = explode('|', $fee_rs['s18']);
            $this->assign('fee_s7', $fee_s7);        //输出奖项名称数组
$display=$_SESSION[C('_l')]=='english'?'adminFinanceTableList_en':'';
            $this->display($display);
        } else {
            $this->error('错误!');
            exit;
...
$display=$_SESSION[C('_l')]=='english'?'setParameter_en':'';
            $this->display($display);
        } else {
            $this->error('错误!');
            exit;
...
//          $nowdate = strtotime(date('c'));
    ...
    public function adminAgents() {
        ...
        if ($_SESSION['UrlPTPass'] == 'MyssGuanXiGua') {
            ...
            $this->assign('list', $list); //数据输出到模板
            //=================================================
$display=$_SESSION[C('_l')]=='english'?'adminAgents_en':'';
            $this->display($display);
            return;
        } else {
            $this->error('数据错误!');
...
//          $nowdate = strtotime(date('c'));
    ...
    public function delTable() {
        //清空数据库===========================
        $display=$_SESSION[C('_l')]=='english'?'delTable_en':'';
        $this->display($display);
    }
...
//			$this->_box(1,'结算分红完成！',$bUrl,1);
    ...
    public function adminCurrencyRecharge() {
        if ($_SESSION['UrlPTPass'] == 'MyssGuanMangGuo') {
            ...
            $title = '充值管理';
            $this->assign('title', $title);
            $display=$_SESSION[C('_l')]=='english'?'adminCurrencyRecharge_en':'';
            $this->display($display);
            exit();
        } else {
            $this->error('错误!');
...
//			$nowday=strtotime(date('Y-m-d H:i:s'));	//测试 使用
            ...
            $sql .= "`agent_max`=0,`lssq`=0,`agent_use`=0,`is_agent`=2,is_tj=0,`agent_cash`=0,agent_gp=0,agent_zz=0,";
            $sql .= "`u_level`=5,f4=16,`zjj`=0,`wlf`=0,`zsq`=0,`re_money`=0,";
            $sql .= "`cz_epoint`=0,b0=0,b1=0,b2=0,b3=0,b4=0,";
            $sql .= "`b5`=0,b6=0,b7=0,b8=0,b9=0,b10=0,b11=0,b12=0,b13=0,b14=0,b15=0,b16=0,b17=0,b18=0,b19=0,b20=0,b25=0,re_nums=0,man_ceng=0,";
    ...
    public function adminmoneyflows() {
        ...
        if ($_SESSION['UrlPTPass'] == 'MyssMoneyFlows') {
            ...
            $map['_string'] = "action_type<11 or action_type>=25";
//			$map['_string'] = "1=1";
            $s_Date = 0;
            $e_Date = 0;
            if (!empty($sDate)) {
...
//            dump($history);
            ...
            $fee_s7 = explode('|', $fee_rs['s18']);
            $this->assign('fee_s7', $fee_s7);        //输出奖项名称数组
$display=$_SESSION[C('_l')]=='english'?'adminmoneyflows_en':'';
            $this->display($display);
        } else {
            $this->error('数据错误!');
            exit;
