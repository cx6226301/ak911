...
//				exit;
//			}
			if(trim($_POST['wt1'])==''||trim($_POST['da1'])==''){
				$this->error('密保问题和答案不能为空！');
				exit;
			}
    ...
    public function changePwd(){
		...
		$myw = array();
		$myw['id'] = $_SESSION[C('USER_AUTH_KEY')];
		$mrs = $fck->where($myw)->field('id,wenti_dan,da1,da2')->find();
		if($wt==1){
		if(trim($mrs['da1'])!=trim($_POST['da'])){
		$this->error('答案错误!');
			exit;
		}
		}else if($wt==2){
		if(trim($mrs['da2'])!=trim($_POST['da'])){
		$this->error('答案错误!');
			exit;
		}
		}
