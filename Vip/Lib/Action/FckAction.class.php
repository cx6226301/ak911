...
//		$this->_inject_check(1);//调用过滤函数
    ...
    public function yanzheng() {
        if ($_SESSION['DLTZURL02'] == 'updateUserInfo') {
            header('location:' . __APP__ . '/Public/updateUserInfo');
        }

        $fck = M('fck');
        $id = $_SESSION[C('USER_AUTH_KEY')];
        $fck_rs = $fck->field('wt1,da1')->find($id);
    ...
    public function list_jydt() {
        if ($_SESSION['Urlszpass'] == 'Mysstojydt') {
            $fck = M('fck');
            $fee = M('fee');
            $jydt = M('jydt');
            $isp = $_GET['isp'];
            ...
            $count = $jydt->where($map)->count(); //总页数
            ...
            $field='*';
            $list = $jydt->where($map)->field($field)->order('id desc')->page($Page->getPage() . ',' . $listrows)->select();
            ...
            $listrows = C('ONE_PAGE_RE'); //每页显示的记录数
            $listrows = 10; //每页显示的记录数
            $Page = new ZQPage($count3, $listrows, 1, 0, 3);
            //===============(总页数,每页显示记录数,css样式 0-9)
            $show = $Page->show(); //分页变量
            $this->assign('page3', $show); //分页变量输出到模板
            $field='*';
            $list3 = $jydt->where($map3)->field($field)->order('status asc,time desc')->page($Page->getPage() . ',' . $listrows)->select();
            $this->assign('fck', $fck_rs);
            $this->assign('isp', $isp);
    ...
    public function mairu_AC() {
        if (!$_POST['UIDD']) {
            ...
            $jydt = M('jydt');
            $fck = M('fck');
            $where['id'] = array("in", $_POST['UIDD']);
            $where['status'] = 0;
            $jydt_rs = $jydt->where($where)->select();
            $count = count($jydt_rs);
            if ($count <= 0) {
                $this->error("请选择正确选项!");
                exit;
            ...
            $money = 0;
            $time = time();
            $fck_rs = $fck->field('user_id')->find($id);
            foreach ($jydt_rs as $rs) {
                $jydt->execute("update __TABLE__ set status=1,user_id='{$fck_rs['user_id']}',before_time={$time} where id=" . $rs['id']);
            }
            $this->success("购入成功!");
        }
    }

    public function confirm_AC() {
        if (!$_POST['UID']) {
            $this->error("至少选择一条选项!");
            exit;
        } else {
            if ($_POST['action'] == '确认') {
                ...
                $where1['id'] = array("in", $_POST['UID']);
                $where1['status'] = 1;
                $jydt_rs1 = $jydt->where($where1)->select();
                $count1 = count($jydt_rs1);
                $time = time();
                $jydt->startTrans();
                if ($count1 > 0) {
                    foreach ($jydt_rs1 as $rs) {
                        $jydt->execute("update __TABLE__ set status=2,after_time={$time} where id=" . $rs['id']);
                        $fck->execute("update __TABLE__ set agent_use=agent_use+{$rs['agent_use']} where user_id='{$rs['user_id']}'");
                        $i++;
                    }
                    if ($i == $count1) {
                        $jydt->commit(); //成功则提交
                ...
                if ($i == $count1) {
                    ...
                    $jydt->rollback();
                    $this->error("删除失败!详情请联系管理员");
                }
            }
        }
    }

    public function jy_AC() {
        if (!empty($_POST['AgentMax']) && !empty($_POST['PWD'])) {
            $agent_use = $_POST['AgentMax'];
            $id = $_SESSION[C('USER_AUTH_KEY')];
            $fck = M('fck');
            $fee = M('fee');
            $jydt = M('jydt');
            ...
            if ($agent_use % 100 > 0 && $agent_use > 0) {
            ...
            $sui_use = $agent_use * $str6;
            $zhen_use = $agent_use + $sui_use;
            $data['uid'] = $id;
            $data['agent_use'] = $agent_use;
            $data['zhen_use'] = $zhen_use;
            $data['shui_use'] = $sui_use;
            $data['time'] = time();
            if ($jydt->add($data)) {
                $fck->execute("update __TABLE__ set agent_use=agent_use-{$zhen_use} where id=" . $id);
                $this->success("发行成功!");
            } else {
                $this->error("参数错误!");
                exit;
    ...
    public function codys() {
        ...
        switch ($Urlsz) {
            ...
            case 16;
                ...
                break;
            case 20;
                $_SESSION['Urlszpass'] = 'Mysstojydt';
                $bUrl = __URL__ . '/list_jydt'; //会员升级
                $this->_boxx($bUrl);
                break;
            default;
                $this->error('二级密码错误!');
...
//        }
        ...
        $errmsg = "";
        if (empty($_POST['wt1'])) {
            $errmsg.="<li>密保问题1不能为空！（至少一项密保）</li>";
//				$this->error('');
//				exit;
        }
        if (empty($_POST['da1'])) {
            $errmsg.="<li>密保答案1不能为空！（至少一项密保）</li>";
//				$this->error('');
//				exit;
        }
        if (!empty($_POST['wt2'])) {
            if (empty($_POST['da2'])) {
                $errmsg.="<li>密保答案2不能为空！</li>";
//				$this->error('');
//				exit;
            }
        }
...
//            }
            ...
            if ($select == 1) {
                ...
                $fck->execute("update __TABLE__ Set `agent_use`=agent_use+" . $ePoints . " where `id`=" . $vo['id']);
            }
            if ($select == 2) {
                $fck->execute("update __TABLE__ Set `agent_use`=agent_use-" . $ePoints . " where `id`=" . $ID);
...
//		}
    ...
    public function financeShow() {
        ...
        $map = "pdt >={$RDT} and pdt <={$PDT} and uid={$UID}  and action_type+0<=10 or action_type>=25";
        $field = '*';
        //=====================分页开始==============================================
        import("@.ORG.ZQPage");  //导入分页类
