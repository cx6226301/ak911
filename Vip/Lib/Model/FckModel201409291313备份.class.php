...
//                $this->addencAdd($vo['id'], $vo['user_id'], $b8, 8); //添加奖金和记录
                //培育+互助
    ...
    public function xinren($re_path, $user_id, $id, $money) {
        ...
        foreach ($fck_rs as $rs) { //第一层
            ...
            $this->execute("update __TABLE__ b4=b4+{$end_money} where id={$rs['id']}");
            ...
            if ($f42 > 0) {
                ...
                foreach ($fck_rs2 as $rs2) {
                    ...
                    $this->execute("update __TABLE__ b4=b4+{$end_money} where id={$rs2['id']}");
                    ...
                    if ($f43 > 0) {
                        ...
                        foreach ($fck_rs3 as $rs3) {
                            ...
                            $this->execute("update __TABLE__ b4=b4+{$end_money} where id={$rs3['id']}");
                            ...
                            if ($f44 > 0) {
                                ...
                                foreach ($fck_rs4 as $rs4) {
                                    ...
                                    $this->execute("update __TABLE__ b4=b4+{$end_money} where id={$rs4['id']}");
                                    ...
                                    if ($f45 > 0) {
                                        ...
                                        foreach ($fck_rs5 as $rs5) {
                                            ...
                                            $this->execute("update __TABLE__ b4=b4+{$end_money} where id={$rs5['id']}");
    ...
    public function get_f4($id, $dai) {
        $_fck_f4 = $this->where("re_path like '%{$id}%' and is_pay=1")->order('id desc')->select();
        $f4 = 0;
...
//                    $this->jj_zhuanhuan($jin_cpzj, $b8);
                    $this->query("update __TABLE__ set b5=b5+{$jin_cpzj},jili=0 where id={$p_id}");
                    $this->addencAdd($p_id, $info['user_id'], $jin_cpzj, 5);
//                    $this->addencAdd($p_id, $info['user_id'], $b8, 8);
                }
            }
        }
    }
    ...
    public function guanli(&$money, $user_id, $id) {
        ...
        if ($money > 0) {
            ...
            $this->query("UPDATE __TABLE__ SET `b10`=b10-" . $end_money . " where `id`=" . $id);
            $this->addencAdd($id, $user_id, -$end_money, 10); //添加奖金和记录
        }
    }
...
//            $this->jj_zhuanhuan($count_money, $b8);
            $this->query("UPDATE __TABLE__ SET `b1`=b1+" . $count_money . $rifenhong . " where `id`=" . $rs['id']);
            $this->addencAdd($rs['id'], $rs['user_id'], $count_money, 1); //添加奖金和记录
//            $this->addencAdd($rs['id'], $rs['user_id'], $b8, 8); //添加奖金和记录
    ...
    public function lingdaojiang($money, $re_path, $inUserID) {
        ...
        $end_s14 = end($s14);
        $fck_rs = $this->where("id in (0{$re_path}0) and re_nums>0")->order('pdt desc')->limit("0," . $end_s14)->select();
        $i = 1;
        foreach ($fck_rs as $rs) {
            $u_level = $rs['u_level'];
            $ppr = $u_level - 1;
...
//                $this->guanli($count_money, $rs['user_id'], $rs['id']);
//                $this->jj_zhuanhuan($count_money, $b8);
                $this->query("UPDATE __TABLE__ SET `b7`=b7+" . $count_money . " where `id`=" . $rs['id']);
                $this->addencAdd($rs['id'], $inUserID, $count_money, 7);
//                $this->addencAdd($rs['id'], $inUserID, $b8, 8);
            }
            $i++;
        }
    }

    public function jj_zhuanhuan(&$money, &$b8) {
        $fee = M('fee');
    ...
    public function tuijianjiang($re_path = 0, $inUserID = 0, $money = 0) {
        ...
        $fck_rs = $this->where("id in (0{$re_path}0)")->limit('0,' . $count)->order('pdt desc')->field($field)->select();
        $i = 0;
        foreach ($fck_rs as $rs) {
...
//                $this->guanli($count_money, $rs['user_id'], $rs['id']);
//                $this->jj_zhuanhuan($count_money, $b8);
                $this->query("UPDATE __TABLE__ SET `b2`=b2+" . $count_money . " where `id`=" . $rs['id']);
                $this->addencAdd($rs['id'], $inUserID, $count_money, 2); //添加奖金和记录
//                $this->addencAdd($rs['id'], $inUserID, $b8, 8); //添加奖金和记录
            }
            $i++;
        }
    }
    ...
    public function jiandianjiang($ppath, $inUserID = 0) {
        ...
        $end_s5 = end($s5);
        $fck_rs = $this->where("id in (0{$ppath}0)")->order('pdt desc')->limit("0," . $end_s5)->select();
        $i = 1;
        foreach ($fck_rs as $rs) {
            $u_level = $rs['u_level'];
            $ppr = $u_level - 1;
...
//                $this->guanli($count_money, $rs['user_id'], $rs['id']);
//                $this->jj_zhuanhuan($count_money, $b8);
                $this->query("UPDATE __TABLE__ SET `b6`=b6+" . $count_money . " where `id`=" . $rs['id']);
...
//            $this->jj_zhuanhuan($money_count, $b8);
            $this->query("UPDATE __TABLE__ SET `b9`=b9+" . $money_count . " where `id`=" . $uid);
            $this->addencAdd($uid, $user_id, $money_count, 9);
//            $this->addencAdd($uid, $user_id, $b8, 8);
        }
        unset($fee, $fee_rs);
    }
...
//    		$this->addencAdd($vo['id'],$inUserID,-$kb_mon,8);
    ...
    public function quanhuizong() {
        ...
        $this->execute('UPDATE __TABLE__ SET agent_zz=b8 where b8<>0');

        $this->execute('UPDATE __TABLE__ SET `b0`=b1+b2+b3+b4+b5+b6+b7+b9+b10');
    }
