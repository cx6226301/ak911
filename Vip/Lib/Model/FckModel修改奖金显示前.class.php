...
//            }
            ...
            if ($money > 0) {
                ...
                $this->query("UPDATE __TABLE__ SET `b3`=b3+" . $money . ",day_feng=day_feng+" . $money . ",b8=b8+{$b8} where `id`=" . $vo['id']);
        ...
        if ($treeplace == 1 && $p_level >= 4 && $jili == 1) { //该会员有右区间-大于等于第四层-还未曾获得
            ...
            if ($count >= 2) {
                ...
                if ($i == 4) {
                    ...
                    $this->query("update __TABLE__ set b5=b5+{$jin_cpzj},jili=0,b8=b8+{$b8} where id={$p_id}");
    ...
    public function rifenhong() {
        ...
        foreach ($fck_rs as $rs) {
            ...
            $this->query("UPDATE __TABLE__ SET `b1`=b1+" . $count_money . $rifenhong . ",b8=b8+{$b8} where `id`=" . $rs['id']);
    ...
    public function lingdaojiang($money, $re_path, $inUserID) {
        ...
        foreach ($fck_rs as $rs) {
            ...
            if ($s14[$ppr] >= $i) {
                ...
                $this->guanli($count_money, $rs['user_id'], $rs['id']);
                $this->jj_zhuanhuan($count_money, $b8);
                $this->query("UPDATE __TABLE__ SET `b7`=b7+" . $count_money . ",b8=b8+{$b8} where `id`=" . $rs['id']);
    ...
    public function tuijianjiang($re_path = 0, $inUserID = 0, $money = 0) {
        ...
        foreach ($fck_rs as $rs) {
            ...
            if ($count_money > 0) {
                ...
                $this->guanli($count_money, $rs['user_id'], $rs['id']);
                $this->jj_zhuanhuan($count_money, $b8);
                $this->query("UPDATE __TABLE__ SET `b2`=b2+" . $count_money . ",b8=b8+{$b8} where `id`=" . $rs['id']);
    ...
    public function jiandianjiang($ppath, $inUserID = 0) {
        ...
        foreach ($fck_rs as $rs) {
            ...
            if ($s5[$ppr] >= $i) {
                ...
                $this->guanli($count_money, $rs['user_id'], $rs['id']);
                $this->jj_zhuanhuan($count_money, $b8);
                $this->query("UPDATE __TABLE__ SET `b6`=b6+" . $count_money . ",b8=b8+{$b8} where `id`=" . $rs['id']);
    ...
    public function baodanfei($uid, $user_id = 0, $money = 0) {
        ...
        if ($money_count > 0) {
            ...
            $this->query("UPDATE __TABLE__ SET `b9`=b9+" . $money_count . ",b8=b8+{$b8} where `id`=" . $uid);
